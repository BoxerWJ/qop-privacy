...
Voyage Ledger ("the App") is a decision-support and investment journaling application.
...
- The backend primarily handles subscription verification, usage control, and secure session authentication.
...
Although the App does not require personal information such as name or email, limited technical information may be processed for service functionality.
...
Payment information (e.g., credit card data) is not collected or stored by the App.  
...
- The server forwards the request to the OpenAI API.
- The backend does not permanently store prompt contents.
- AI responses are returned to the user and may be stored locally on the device.
...
- Session and subscription status data are retained only as necessary for service operation.
- Decision logs remain on the user’s device unless manually exported.
- Server logs may be retained for operational security and abuse prevention.
...
Voyage Ledger(투자 항해일지) (“이 앱”)은 투자 의사결정 지원 및 기록 관리 애플리케이션입니다.

이 앱은 **프라이버시 우선 구조(Privacy-First Architecture)** 로 설계되었습니다.
...
- 세션 및 구독 상태 정보는 서비스 운영에 필요한 기간 동안만 유지됩니다.
